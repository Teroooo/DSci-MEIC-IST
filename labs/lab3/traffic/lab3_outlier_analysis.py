...
def plot_results(results_list, best_res):
    """Generates the two required charts."""
    ...
    ax.set_title('Impact of Outlier Removal')
    ax.set_xticks(x)
...
def main():
    ...
    # Contamination=0.01 means we guess 1% of data are outliers
    iso = IsolationForest(contamination=0.01, random_state=42, n_jobs=-1)
    
    # Identify outliers in TRAIN only
    print("   -> Detecting outliers...")
    outlier_labels = iso.fit_predict(X_train)
    
    # -1 means outlier, 1 means inlier. We keep only 1s.
    mask = outlier_labels == 1
    X_train_clean = X_train[mask]
    y_train_clean = y_train[mask]
    
    print(f"   -> Removed {sum(outlier_labels == -1)} outliers from training set.")
